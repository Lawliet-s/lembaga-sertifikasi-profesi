...
@section('isi')
    <div class="page-header">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-custom bg-danger">
                ...
                <li style="color: #fff" class="breadcrumb-item active" aria-current="page">Validasi Kompetensi</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="card">
        <div class="card-header bg-danger text-white d-flex justify-content-between align-items-center">
            <h6 class="mb-0"><i class="fas fa-list mr-2"></i>Daftar Peserta Validasi</h6>
                    ...
                    <tbody>
                        @forelse ($participants as $i => $item)
                            @php
                                ...
                                $statusValidasi = $vd['status_akhir'] ?? ($item->status ?? 'Belum Validasi');
                                $allChecked = !empty($vd['checklist']['bukti_lengkap'])
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($vd && $allChecked)
                                        <span class="badge badge-success">{{ $statusValidasi }}</span>
                                    @elseif ($vd && !$allChecked)
                                        <span class="badge badge-danger">{{ $statusValidasi }}</span>
                                    @else
                                        <span class="badge badge-secondary">Belum Validasi</span>
                                    @endif
                                </td>
                                <td>
                                    <a href="{{ route('asesor.validasi.show', $item->id) }}"
